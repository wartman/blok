...
class DomEngine implements Engine {
  ...
  final styleEngine:DomStyleEngine = new DomStyleEngine();
	...
	public function updateNodeAttr(node:Node, key:String, oldValue:Dynamic, newValue:Dynamic) {
    ...
    for (s in style) {
      styleEngine.define(s.getName(), () -> s);
    }
  }
