      ...
      nameBuilder.push(
        macro $i{PROPS}.$name != null 
          ? ${ Context.unify(type.toType(), Context.getType('blok.internal.VStyle.Unit')) 
              ...
              : Context.unify(type.toType(), Context.getType('blok.internal.VStyle.ValueDef'))
      ...
      return [
        {
          ...
          pos: cls.pos,
          kind: FFun({
            ret: macro:blok.internal.VStyle,
...

        {
          ...
          pos: cls.pos,
          kind: FFun({
            ret: macro:blok.internal.VStyle.VStyleExpr,
            params: cls.params.length > 0
              ? [ for (p in cls.params) { name: p.name, constraints: [] } ]
              : [],
            args: [
